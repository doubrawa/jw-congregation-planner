...
begin
  ...
  elsif art = 'helper' and n = 5 and teile[5] ~ '^\d+
...
create policy notifications_insert on public.notifications
  for insert with check (
    ...
    and (
      public.is_planner()
      -- Verhinderungs-Meldungen gehen an die Planer, nicht an beliebige
      -- Mitglieder (migration-022).
      or (
        type = 'verhindert'
        and exists (
...
create policy confirmations_write on public.confirmations
  ...
  with check (
    congregation_id = public.my_congregation_id()
    and user_id = auth.uid()
    -- Nicht nur die eigene Zeile, auch die eigene Aufgabe (migration-022, T89).
...
-- =============================================================================
 then
  ...
  elsif art in ('part', 'aux') and n = 5 and teile[5] ~ '^\d+
-- Sie dürfen die Treffpunkte pflegen, ohne volle Planer-Rechte zu haben; die
-- Einschränkung auf die eigene Gruppe macht die App.
create or replace function public.is_group_overseer()
returns boolean

 ...
 *
 * **Warum das hier liegt und nicht mehr in `src/i18n/`** (verschoben am
 * 28.8.2026): Eine Push-Erinnerung ist fertiger Text, sobald sie das Gerät
 * erreicht — anders als die Glocke in der App lässt sie sich beim Anzeigen
 * nicht mehr übersetzen. Ihr **Titel** wurde deshalb längst serverseitig
 * übersetzt (`send-reminders/texte.ts`), ihr **Rumpf** aber nicht: Er ging
 * kanonisch deutsch hinaus, an alle 34 Sprachen. Ein koreanischer Verkündiger
 * las einen koreanischen Titel über einem deutschen Rumpf.
 *
 * Der Grund war eine Trennung, keine Absicht: Die Edge-Laufzeit kommt nicht an
 * `src/` heran. `_shared/` ist die vorgesehene Ablage dafür — und der Client
 * bindet **dieselbe Datei** ein (`src/i18n/translate.ts` reicht sie getypt
 * weiter). Keine zweite Abschrift, kein Auseinanderlaufen.
 *
 * `code` ist hier bewusst `string` und nicht `Lang`: Der Typ wohnt im Client
 * (`src/data/types.ts`), und diese Datei muss auch unter Deno übersetzen. Die
 * engere Signatur setzt `src/i18n/translate.ts` wieder auf.
 */
...
export async function bibelbuecherLaden(): Promise<boolean> {
  ...
  const m = await import('./bible-books.ts')
...
function makeTrIntl(code: string): (s: string) => string {

...
/**
 * Wo tsc liegt — aufgelöst, wie Node selbst Pakete findet: von `scripts/` aus
 * aufwärts bis zum ersten `node_modules`, das TypeScript enthält.
 *
 * **Kein fester Pfad `<wurzel>/node_modules/…`.** Ein Worktree unter
 * `.claude/worktrees/…` hat kein eigenes `node_modules` mit Paketen, nur die
 * Cache-Ordner von vite und vitest. `npx` findet TypeScript durch Hochwandern
 * im Hauptcheckout; der feste Pfad, der bis zum 26.9.2026 hier stand, fand es
 * nicht (was daraus wurde, steht bei `tscBefund`).
 *
 * `undefined`, wenn TypeScript von hier aus nirgends zu finden ist.
 */
export function tscPfad() {
  try {
...
function messen() {
  const tsc = tscPfad()
  if (!tsc) return { fehler: 'TypeScript ist von scripts/ aus nicht zu finden — fehlt `npm ci`?' }

...
describe('parseKuratiert', () => {
  const SQL = [
    ...
    PERSON_ZEILE,
    "insert into public.persons (id, congregation_id, fn, ln, dn, role, female, tel, mail, absent, priv, grp, fam) values " +
      "('0279161b-80c9-4d0f-8500-f54c8dd62a9a', (select id from public.congregations limit 1), 'Philemon', 'Weißbrodt', '', " +
  ...
  it('trennt Gruppen, Personen und Aufseher/Gehilfe', () => {
    ...
    expect(k.persons[1].ln).toBe('Weißbrodt') // Umlaut unversehrt
...
describe('displayName', () => {
  it('nimmt den Kurznamen, sonst Vor- und Nachname', () => {
    expect(displayName('Jürgen', 'Doubrawa', '')).toBe('Jürgen Doubrawa')

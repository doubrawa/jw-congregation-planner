...
import { istSyntaxfehler, tscBefund, tscPfad } from './check-index-access.mjs'
...
describe('tscPfad', () => {
  it('findet tsc, wie Node es findet — auch im Worktree ohne eigene Pakete', () => {
    // Dort liegt TypeScript nur im Hauptcheckout darüber; der feste Pfad
    // `<wurzel>/node_modules/…` zeigte ins Leere.
    const pfad = tscPfad()
    expect(pfad !== undefined && existsSync(pfad), `tsc nicht gefunden: ${pfad}`).toBe(true)
  })
})

...
/** Zusammenkünfte Di 19:00 und So 10:00; Woche 0 beginnt am Montag, 7.9.2026. */
...
function woche(patch: Partial<Week> = {}): Week {
  ...
  return { range: '', book: '', start: '2026-09-07', mid: { ...leer }, we: { ...leer }, ...patch }
}
...
describe('Abwesenheiten auf Wochen abbilden', () => {
  it('trifft die Woche, in der die Zusammenkünfte liegen', () => {
    ...
    const set = buildAbsences([abw({ from: '2026-09-07', to: '2026-09-13' })], wochen(3), MEETINGS)
  ...
  it('sperrt nur die getroffene Zusammenkunft, nicht die ganze Woche', () => {
    ...
    const set = buildAbsences([abw({ from: '2026-09-12', to: '2026-09-13' })], wochen(3), MEETINGS)
    expect(istAbwesend(set, 'p1', 0, 'mid')).toBe(false)
    expect(istAbwesend(set, 'p1', 0, 'we')).toBe(true)
  })

  it('deckt mehrere Wochen ab', () => {
    const set = buildAbsences([abw({ from: '2026-09-07', to: '2026-09-27' })], wochen(5), MEETINGS)
  ...
  it('nimmt Anfangs- und Endtag mit (einschließlich)', () => {
    const nurDienstag = buildAbsences([abw({ from: '2026-09-08', to: '2026-09-08' })], wochen(2), MEETINGS)
  ...
  it('bleibt am selben Datum, wenn andere Wochen geladen sind', () => {
    ...
    const spaeter = buildAbsences([eintrag], wochen(3, 2), MEETINGS)
  ...
  it('der Termin kommt aus dem Startdatum der Woche, nicht aus ihrer Position', () => {
    // Eine einzelne Woche im November: ihr Dienstag ist der 3.11., gleich
    // welchen Index sie im Bestand hat.
    const importiert = [woche({ start: '2026-11-02' })] // Mo 2.11. → Di 3.11.
    const set = buildAbsences([abw({ from: '2026-11-03', to: '2026-11-03' })], importiert, MEETINGS)
    expect(istAbwesend(set, 'p1', 0, 'mid')).toBe(true)
  })
  ...
  it('folgt der Abweichung der Woche (Gedächtnismahl)', () => {
    ...
    const set = buildAbsences([abw({ from: '2026-09-12', to: '2026-09-12' })], sonder, MEETINGS)
    expect(istAbwesend(set, 'p1', 0, 'we')).toBe(true)
  })

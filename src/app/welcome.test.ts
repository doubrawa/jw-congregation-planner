...
describe('welcomeDecision', () => {
  it('begrüßt mit dem Vornamen der angemeldeten Person', () => {
    expect(welcomeDecision(true, 'ready', 'Jörg')).toEqual({ name: 'Jörg' })
  })
  ...
  it('wartet, solange die Daten noch laden', () => {
    ...
    expect(welcomeDecision(true, 'loading', 'Jörg')).toBe('warten')
  })
  ...
  it('ohne Vormerkung passiert nichts', () => {
    ...
    expect(welcomeDecision(false, 'ready', 'Jörg')).toBe('warten')
  })

...
export function WeekChips({ week, showCurrent }: { week: Week; showCurrent: boolean }) {
  ...
  if (showCurrent && week.current)
    chips.push({ key: 'cur', label: 'AKTUELLE WOCHE', cls: 'week-chip--current' })
  if (week.co) chips.push({ key: 'co', label: 'BESUCH DES KREISAUFSEHERS', cls: 'week-chip--co' })
  if (week.mem) chips.push({ key: 'mem', label: 'GEDÄCHTNISMAHL', cls: 'week-chip--mem' })
...
export function MemorialBanner({ week, tab }: { week: Week; tab: MeetingTab }) {
  ...
  const cancelled = week.memCancel === 'we' ? 'Wochenende' : 'Unter der Woche'
  const date = (week.memCancel === 'we' ? week.we : week.mid).date
  return (
    <div className="mem-banner">
      Die Zusammenkunft „{cancelled}“ entfällt in dieser Woche — stattdessen findet das
      Gedächtnismahl statt:
      <div className="mem-banner-date">{date}</div>
    </div>
  )
}
